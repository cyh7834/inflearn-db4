...
하이브리드 전략을 쓰려면 운영 규칙이 필요하다.

1. 새 코드 추가는 개발자가 담당한다.
   새 상태 코드는 비즈니스 로직 변경을 의미하므로 ENUM과 공통 코드 테이블을 함께 추가해야 한다.

2. 표시 이름과 속성 변경은 운영자가 담당한다.
   표시 이름, 할인율, 수수료율 등은 어드민 도구에서 변경해 즉시 반영할 수 있다.

3. ENUM과 DB 동기화를 검증한다.
   ENUM에 정의된 코드와 공통 코드 테이블의 코드가 항상 일치해야 한다. 테스트 케이스나 애플리케이션 시작 시 검증으로 누락을 잡는 것이 안전하다.

## 선택 가이드

| 상황 | 선택 |
| --- | --- |
| 화면 표시나 드롭다운 목록이 목적이고 로직 분기가 없음 | 공통 코드 테이블 |
| 비즈니스 로직에서 분기하지만 표시 이름이나 속성 변경이 거의 없음 | 애플리케이션 ENUM |
| 비즈니스 로직 분기도 필요하고 표시 이름이나 속성 변경도 잦음 | 하이브리드 전략 |

의사결정 흐름은 다음과 같다.

1. 이 코드가 `if`, `switch` 등 비즈니스 로직에 사용되는가?
   아니면 공통 코드 테이블만 사용한다.

2. 비즈니스 로직에 사용되지만 표시 이름이나 속성이 거의 바뀌지 않는가?
   그렇다면 ENUM만 사용한다.

3. 비즈니스 로직에 사용되고 표시 이름이나 속성도 운영 중 자주 바뀌는가?
...
### 공통 코드 외래 키를 자주 걸지 않는 이유

문서에서는 실무에서 공통 코드에 외래 키를 잘 걸지 않는 이유도 설명한다.

첫째, 상세 코드의 PK가 `(group_code, code)` 복합 키라서 참조 테이블에도 `group_code` 컬럼이 필요해진다. 예를 들어 주문 상태를 FK로 강제하려면 모든 주문 행에 `ORDER_STATUS`라는 같은 그룹 코드를 반복 저장해야 한다.

둘째, 대량 트랜잭션 테이블에 FK 검증 비용이 추가된다. 공통 코드는 보통 애플리케이션 레벨, 특히 ENUM이나 검증 로직에서 통제하는 경우가 많으므로 DB에서는 느슨하게 유지하는 선택을 할 수 있다.

다만 이 판단은 시스템의 정합성 요구 수준에 따라 달라질 수 있다. 공통 코드가 강한 DB 무결성을 요구하는 도메인이라면 FK 적용도 별도로 검토해야 한다.

## 최종 정리

공통 코드 설계의 목표는 반복되는 코드성 데이터를 일관되게 관리하면서 운영 변경에 유연하게 대응하는 것이다. 기본 설계는 `common_code_group`과 `common_code_detail`을 분리하고, `(group_code, code)` 복합 키로 그룹 내 유일성을 보장하는 방식이다.

공통 코드의 가장 큰 비용은 조인 증가와 SQL 복잡도다. 이를 줄이려면 애플리케이션에서 코드값을 이름으로 매핑하고, 공통 코드 특성에 맞게 로컬 캐시와 TTL을 적용하는 전략이 유용하다.

설계 선택은 목적에 따라 나눈다.

- 단순 표시용은 공통 코드 테이블
- 비즈니스 로직 분기용은 ENUM
- 로직 안전성과 운영 유연성이 모두 필요하면 하이브리드 전략

잘 설계된 공통 코드는 단순한 코드 목록을 넘어, 시스템 전체의 유지보수성과 운영 효율성을 높이는 기반 설계가 된다.